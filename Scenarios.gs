 ...
 * 外線対応ロープレのシナリオデータ定義。
 *
 * GAS は全 .gs ファイルがグローバルスコープを共有するため、
 * ここで定義した関数は Code.gs から直接呼び出せる。
 *
 * 各シナリオのフィールド:
 *   id          : シナリオ識別子（例: "S01"）
 ...
 *   caller_name : 架電者の氏名
 *   company     : 架電者の会社・業種
 *   position    : 架電者の立場（役職や担当）
 *   purpose     : 用件
 *   personality : 性格・口調（システムプロンプトに渡す）
 *   first_message : AIの第一声（架電者の最初のセリフ）
 */
...
var SCENARIOS = [
  ...
  },
  {
    ...
    personality: '取引のある会社なので落ち着いている。用件は「田中さんに代わってほしい」だけで明確。愛想は良い。',
    first_message: 'お世話になっております。中央ロジスティクスの佐藤と申します。営業担当の田中様はいらっしゃいますでしょうか？'
  },
  {
    id: 'S03',
    difficulty: 'beginner',
  ...
  // ===== 中級 =====
  {
    ...
    personality: 'やや急ぎ気味。鈴木さんが不在と分かったら折り返しを依頼する。折り返し先の番号と都合の良い時間帯（本日17時まで）を伝える。',
    first_message: 'お世話になります、マルワ商事の伊藤です。鈴木さんお願いしたいんですが、いらっしゃいますか？'
  },
  {
    id: 'S05',
    difficulty: 'intermediate',
  ...
  // ===== 上級 =====
  {
    ...
    personality: '不満が溜まっており語気が強め。「金を払っているのに成果ゼロ」という気持ち。ただし暴言は吐かない。誠実な説明と具体的な改善提案があれば徐々に落ち着く。',
  ...
  },
  {
    ...
    personality: '冷静だが強い口調。「効果がないから解約・返金したい」と最初から要求が明確。規約上すぐ返金できない場合でも、代替案や誠実な対応次第では話を聞く姿勢はある。感情的になりすぎず理詰めで迫る。',
  ...
  },
  {
    ...
    personality: '強い怒りと焦りを抱えている。求人票の時給が「1,500円」のところ「1,800円」と誤って掲載され、応募者から「話が違う」と苦情が来ている。誰の責任か、どう収拾するのか、再発防止はどうするのかを問い詰める。誠実な謝罪と具体的な対応策で徐々に落ち着く。',
